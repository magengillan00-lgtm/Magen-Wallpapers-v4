...
class HomeWallpaperService: Service() {
    ...
    private fun workerTaskRefresh() {
        serviceScope.launch(Dispatchers.Default) {
            refreshAlbumSuspend(this@HomeWallpaperService)
            withContext(Dispatchers.Main) {
                stopSelf()
            }
        }
    ...
    private suspend fun changeWallpaper(context: Context) {
        try {
            ...
            if (settings.skipLandscape && context.resources.configuration.orientation == android.content.res.Configuration.ORIENTATION_LANDSCAPE) {
                Log.d("AutoWallpaperChangerPro", "Skipping wallpaper change - device is in landscape mode")
                stopSelf()
                return
            }
            ...
            if (settings.skipNonInteractive){
                ...
                if (!powerManager.isInteractive){
                    Log.d(
                        "AutoWallpaperChangerPro", "Skipping wallpaper change - device is in non-interactive state")
                    stopSelf()
                    return
                }
            }
            ...
            when {
                settings.setHome && settings.setLock && scheduleSeparately -> {
                    ...
                    if (wallpaper == null) {
                        ...
                        if (wallpaper == null) {
                            Log.d("AutoWallpaperChangerPro", "No wallpaper found")
            ...
            Log.e("AutoWallpaperChangerPro", "Error in changing wallpaper (Home)", e)
        }
    }
    ...
    private suspend fun updateCurrentWallpaper(context: Context) {
        try {
            ...
            Log.e("AutoWallpaperChangerPro", "Error in updating", e)
        }
    }
    ...
    private suspend fun setWallpaper(
        ...
        wallpaper?.let {
            try {
                ...
                Log.e("AutoWallpaperChangerPro", "Error setting wallpaper", e)
                return false
            }
        }
        return false
    ...
    private suspend fun refreshAlbumSuspend(context: Context) {
        ...
        try {
            ...
            albumsToRefresh.forEach { albumWithDetails ->
                ...
                val updatedFolders = albumWithDetails.folders
                    .mapNotNull { folder ->
                        try {
                                ...
                                val newWallpapers = wallpapersOnDisk
                                    ...
                                    .mapIndexed { index, wallpaper ->
                                        wallpaper.copy(
                                            ...
                                            key = "${albumWithDetails.album.initialAlbumName}|${folder.folderUri}|${wallpaper.wallpaperUri}"
                                        )
                                    }
                            ...
                            Log.w("AutoWallpaperChangerPro", "Failed to process folder ${folder.folderUri}: ${e.message}")
                            null
                        }
                    }
                ...
                if (allValidWallpapers.isEmpty()) {
                    Log.d("AutoWallpaperChangerPro", "Album '${albumWithDetails.album.initialAlbumName}' is now empty. Deleting.")
            ...
            Log.e("AutoWallpaperChangerPro", "Error refreshing album", e)
        }
    }
